...
export async function parseText(text: string): Promise<DocumentParseResponse> {
  const response = await fetch("/api/documents/parse-text", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(message || "Text parse failed with " + response.status);
  }
